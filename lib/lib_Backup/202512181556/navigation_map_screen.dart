...
class _NavigationMapScreenState extends State<NavigationMapScreen>
  ...
  int _activeDestinationIndex = 0; // نگه داشته شد
  ...
  void _onMapTapped(LatLng point) {
    if (_isSelectingFromMap) {
      ...
      setState(() {
        ...
        if (_activeDestinationIndex < _destinationControllers.length) {
          _destinationControllers[_activeDestinationIndex].text = coordsText.length > 35 
              ? "${coordsText.substring(0, 35)}..." 
              : coordsText;
        }

        // مارکر فقط برای مقصد اصلی (یا می‌تونی برای همه بذاری)
      ...
      pageBuilder: (_, __, ___) => RoutingTopPanel(
        ...
        onPickFromMap: (int index) {
          setState(() {
            ...
            _isRoutingPanelMinimized = true; // پنل مینیمایز می‌شود
          });
          _showSnackBar("روی نقشه تپ کنید تا نقطه انتخاب شود", success: true);
        },
        // 👈 فقط این خط اضافه شد — تمام!
                        ...
                        children: [
                          Row(
                            ...
                            children: [
                              ...
                              TextButton(
                                onPressed: () async {
                                  await state._historyManager.clearHistory();
                                  state.setState(() {});
                                },
                          ...
                          ...history.take(4).map((query) => _HistoryTile(
                                ...
                                onRemove: () async {
                                  await state._historyManager.removeHistoryItem(query);
                                  state.setState(() {});
                                },
                              )).toList(),
                        ],
